//Original: https://github.com/DavidXanatos/priv10/blob/master/PrivateWin10/API/AuditPolicy.cs 
//reworked by 0xffhh
...
static public class AuditPolicyFetcher
{
    /// <summary>
    ...
    [DllImport("advapi32.dll", SetLastError = true)]
    private static extern bool AuditQuerySystemPolicy(IntPtr pSubCategoryGuids, uint PolicyCount, out IntPtr ppAuditPolicy);
    ...
    static public List<Guid> GetCategoryIdentifiers()
    {
    ...
    static public List<Guid> GetSubCategoryIdentifiers(String categoryGuid)
    {
    ...
    /// <param name="guid">The GUID of the category for which the subcategories should be returned.</param>
    /// <returns>The GUIDs of the audit subcategories for the specified category.</returns>
    static public List<Guid> GetSubCategoryIdentifiers(Guid categoryGuid)
    {
        List<Guid> identifiers = new List<Guid>();
        IntPtr buffer;
    ...
    static public AUDIT_POLICY_INFORMATION GetSystemPolicy(Guid subCategoryGuid)
    {
        ...
        AUDIT_POLICY_INFORMATION policyInformation;
        IntPtr buffer; 
        IntPtr lpSCGuid = Marshal.AllocHGlobal(Marshal.SizeOf(typeof(Guid)));
        Marshal.StructureToPtr(subCategoryGuid, lpSCGuid, false);  
        bool success = AuditQuerySystemPolicy( lpSCGuid, 1, out buffer);
        if (!success) 
        { 
            throw new Win32Exception(Marshal.GetLastWin32Error()); 
        }
